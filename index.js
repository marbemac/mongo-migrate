 ...
 */
var usage = [
    '', '  Usage: migrate [options] [command]', '', '  Options:', '', '     -runmm, --runMongoMigrate   Run the migration from the command line', , '     -c, --chdir <path>    		change the working directory', '     -cfg, --config <path> 		DB config file name', '     -dbn, --dbPropName <string> Property name for database connection in config file', '', '  Commands:', '', '     down   [name]    migrate down till given migration', '     up     [name]    migrate up till given migration (the default command)', '     create [title]   create a new migration file with optional [title]', ''
].join('\n');
...
var template = [
    '', 'var mongodb = require(\'mongodb\');', '', 'exports.up = function(db, next){', '    next();', '};', '', 'exports.down = function(db, next){', '    next();', '};', ''
].join('\n');
...
function runMongoMigrate(direction, migrationEnd, next) {
    ...
    function performMigration(direction, migrateTo, next) {
        ...
        db.getConnection(dbConfig || require(process.cwd() + path.sep + configFileName)[dbProperty], function(err, db) {
            ...
            migrationCollection.find({}).sort({
                num: -1
            }).limit(1).toArray(function(err, migrationsRun) {
                if (err) {
